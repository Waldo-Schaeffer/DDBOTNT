...
import json, requests, math, time, sys
import pymysql
...
class QQBotInfo:
    ...
    def send_group_message(raw_message):
        url = f"{self.base_url}/{api_name}"
        response = requests.post(url=url,data=raw_message, headers=self.headers)
    ...
    def 发送下播通知(self, group_id, anchor_name, cover_visual_url):
        ...
        # 示例：发送消息的代码（请根据实际使用的库或框架调整）
        return self.send_group_message(raw_message)
    
    def 发送开播通知(group_id, anchor_name, live_title, room_number, cover_visual_url,at_list=None, at_all=False):
        """
        发送开播提醒到指定的QQ群。
        ...
        if at_list is None and at_all == False:
            return None
        
        roomurl = "https://live.bilibili.com/" + str(room_number)
        raw_mention_qq = ""  # 初始化为空字符串
        if at_all:
            raw_mention_qq = "[CQ:at,qq=all] "
        elif at_list is not None:
            for item in at_list:
                raw_mention_qq += f"[CQ:at,qq={item}] "
        
        raw_message={
            "group_id": group_id,
            "message":f"{raw_mention_qq}{anchor_name}正在直播【{live_title}】\r\n{roomurl}\r\n[CQ:image,file={cover_visual_url}]"
        }
...
class DDBOTMain():
    ...
    def 推送判定(B站直播列表, 群关注列表):
        # 创建一个 B站直播uids索引，用于快速检查uid是否存在
        B站直播uids索引 = set(item['uid'] for item in B站直播列表)
        
        # 从群关注列表中，根据uid寻找需要推送的群号
        下播推送列表 = [user for user in 群关注列表 if user['uid'] in B站直播uids索引]
...
if __name__ == '__main__':
    ...
    while 1:
        ...
        Q群推送列表 = []
        开播的主播 = DDBOTMain.下播判定(DDBOT开播的主播缓存, 当前群关注的B站开播主播列表)
        下播的主播 = DDBOTMain.下播判定(当前群关注的B站开播主播列表, DDBOT开播的主播缓存)
                ...
                try:
                    for item in 已经开启下播推送的群队列:
                        Res = QQNTBOT.发送下播通知(
                            ...
                            cover_visual_url=item['cover_visual_url']
                        )
                        time.sleep(2) 
                        print(Res)
                except Exception as e:
